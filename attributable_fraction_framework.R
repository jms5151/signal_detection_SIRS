...
# source functions to calculate FAR, RR, AF, and Re
...
af_plot <- ggplot(dfx, aes(x = climate_delta, y = AF, color = threshold_level)) +
  ...
  theme_bw(base_size = 14) +
  facet_wrap(~func, nrow = 1) + 
  labs(title = 'Climate attribution', x = 'Extreme event caused climate deviation', y = 'Attributable fraction') +
  scale_color_manual(
    values = value_colors,
...
# susceptibility plot
suscept_plot <- ggplot(long_data_s, aes(x = climate_delta, y = Re, color = threshold_level, linetype =  Susceptibility)) +
  geom_line(size = 1.1) +
  ...
  scale_color_manual(values = value_colors) + 
  guides(color = 'none') +
  labs(title = 'Susceptibility impacts', x = 'Extreme event caused climate deviation', y = expression(R[e])) +
  labs(color = NULL) +
  geom_hline(yintercept = 1, col = 'grey') +
  theme(legend.position = 'bottom', legend.background = element_rect(fill='transparent')) + #c(.055,.68)
  theme(
...
# Arrange the plots
multiplot <- (functions_plot / af_plot / suscept_plot) + 
  plot_layout(guides = "collect", heights = unit(c(0.8, 1, 1), "null")) & 
